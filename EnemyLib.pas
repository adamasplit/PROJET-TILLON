...
begin
  ...
  InitAnimation(ennemi.anim,ennemi.anim.ObjectName,'warp', 6,False);
  if distx>=0 then
    ennemi.stats.xcible:=900
  else
    ennemi.stats.xcible:=70;
  ennemi.stats.compteurAction:=0;
  ennemi.col.estActif:=False
end;
  ...
  if (ennemi.stats.vie>0) then 
      ...
      if (ennemi.anim.etat='rewarp') and (animFinie(ennemi.anim)) then begin
        ...
        if (ennemi.image.rect.x<joueur.image.rect.x) then
          CreerRayon(typeobjet(1),2,1,1,ennemi.image.rect.x+40,ennemi.image.rect.y+50,ennemi.image.rect.x+60,ennemi.image.rect.y+50,0,10,5,'rayon',obj)
        else
          CreerRayon(typeobjet(1),2,1,1,ennemi.image.rect.x+40,ennemi.image.rect.y+50,ennemi.image.rect.x-60,ennemi.image.rect.y+50,0,10,5,'rayon',obj);
        ajoutObjet(obj);
      ...
      //pour un ennemi qui se téléporte 
			...
			if animFinie(ennemi.anim) and (ennemi.anim.etat='chase') and (random(20)=0)  then begin
