...
begin
  ...
  // Load music
  Music := Mix_LoadMUS('Combat.ogg');
  if Music = nil then Exit;
  Mix_VolumeMusic(MIX_MAX_VOLUME);
  Mix_PlayMusic(OST[1].musique,0);
  // Load sound
  Sound := Mix_LoadWAV('effet.wav');
  if Sound = nil then Exit;
  Mix_VolumeChunk(Sound, MIX_MAX_VOLUME);
  LastUpdateTime3:=SDL_GetTicks;
  while Run = True do
  begin
    bouclerMusique(OST[1],LastUpdateTime3);
